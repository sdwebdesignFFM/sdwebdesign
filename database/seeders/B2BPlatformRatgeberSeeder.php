 ...
 * exist in production. Use this when the equivalent migrations
 * have been registered as run but the data isn't actually present
 * (e.g. after a partial failure during a deploy).
 *
 * Run via:
 *   php artisan db:seed --class=B2BPlatformRatgeberSeeder --force
 *
 * Idempotent: deletes corrupted leftovers, only creates articles
 * that don't already exist (matched by slug->de).
 */
class B2BPlatformRatgeberSeeder extends Seeder
{
    public function run(): void
    {
        $this->cleanupCorruptedSlugs();
        $this->ensureCostArticle();
        $this->ensureAgencyArticle();
    }

    private function cleanupCorruptedSlugs(): void
    {
...
TXT;
        ...
        $this->command?->info('  • created /ratgeber/was-kostet-b2b-plattform');
    }
...
TXT;
        ...
        $this->command?->info('  • created /ratgeber/software-agentur-frankfurt-mittelstand');
    }
}

...
/**
 * Belt-and-suspenders re-creation of the two B2B-platform ratgeber
 * articles. Migration 2026_04_26_193432 must have aborted on the
 * MySQL array-slug issue without persisting either article — both
 * URLs returned 404 after the deploy. The repair migration
 * 2026_04_27_081509 only handled the cost article.
 *
 * This migration:
 *   1. Removes any obviously corrupted leftovers ("was-kostet-b",
 *      bare "b") that the previous repair didn't catch (e.g. via
 *      the plain `slug` column rather than `slug->de`).
 *   2. For each of the two articles, checks slug->de and only
 *      creates if missing — never overwrites editor changes.
 *
 * All translatable fields use plain strings, matching the working
 * BlogArticleSeeder convention.
 */
return new class extends Migration
{
    public function up(): void
    {
        // Defensive cleanup of any partial / corrupted records.
        ...
        $this->ensureCostArticle();
        $this->ensureAgencyArticle();
    }

    public function down(): void
    {
        BlogArticle::where('slug->de', 'was-kostet-b2b-plattform')->delete();
        BlogArticle::where('slug->de', 'software-agentur-frankfurt-mittelstand')->delete();
    }
